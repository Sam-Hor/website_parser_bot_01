...
images_folder = '/Users/Sam/Python_razr/website_parser_bot_01/images'
...
for page in range(1, 14):
    ...
    for i in all_titles:
        ...
        if header_div is not None:
            ...
            image_url = i.find('img').get('src')  # получить URL изображения
            image_name = os.path.join(images_folder, image_url.split('/')[-1])  # получить имя файла изображения
            urllib.request.urlretrieve(image_url, image_name)  # скачать и сохранить изображение
...
header = ['date', 'title', 'author', 'link', 'likes', 'comments', 'notes','images']
